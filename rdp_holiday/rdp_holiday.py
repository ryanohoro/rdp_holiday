...
class RDPHoliday:
    ...
    def __init__(self, rdp_file=None):
        self.MISSED_DEPS = False
        self.rdp_properties = {}

        if rdp_file:
            self.rdp_properties = self.parse_rdp_file(rdp_file)

    def validate_sig(self, hostname, alternate_faddress, pkcs7_certificates):
        sign_data = {
            "can_sign": False,
            "usage_error": "",
            "valid": False,
        ...
        try:
            ...
            certs = [x509.Certificate.load(cert.public_bytes(serialization.Encoding.DER)) for cert in
                     pkcs7_certificates]
                    ...
                    try:
                        ...
                        try:
                            if signature_bytes:
                                ...
                                rdp_properties["certificate_chain_validation"] = self.validate_sig(
                                    f"{rdp_properties['full_address']}", f"{rdp_properties['alternate_full_address']}",
                                    certs
                                )
                                for cert in certs:
                                    cert_info = {
...
def main():
    ...
    parser.add_argument("input_file", help="Input RDP file path")
    parser.add_argument("-o", "--output_file", help="Output file path to save JSON results")
    args = parser.parse_args()

    rdpholiday = RDPHoliday(args.input_file)
